...
IF p-ind-event = "AFTER-INITIALIZE":U THEN DO:
    ...
    IF VALID-HANDLE(wh-nr-pedcli) THEN DO:
        
        ON "ENTRY" OF wh-nr-pedcli PERSISTENT RUN upc/upc-pd4000.p (INPUT "EntryNrPedCli",
                                                                    STRING(wh-nr-pedcli),
                                                                    INPUT p-wgh-object,
                                                                    INPUT p-wgh-frame ,
                                                                    INPUT p-cod-table ,
...
AND p-ind-object = "container" THEN DO:
    ...
    DO WHILE VALID-HANDLE(wgh-grupo) :
        ...
        IF wgh-grupo:TYPE = "FRAME"
            AND wgh-grupo:NAME = "fPage10" THEN DO:
               ASSIGN wgh-frame = wgh-grupo:HANDLE.
...
IF p-ind-event = 'EntryNrPedCli' THEN DO:
