...
*******************************************************************************/
{include/i-prgvrs.i esnf016-g01 2.06.00.001}
...
&Scoped-Define ENABLED-OBJECTS rt-button c_cod-cfop bt-ok bt-cancela ~
bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS c_cod-cfop 
...
DEFINE VARIABLE c_cod-cfop AS CHARACTER FORMAT "x(10)":U 
     LABEL "CFOP" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     c_cod-cfop AT ROW 1.17 COL 18 COLON-ALIGNED WIDGET-ID 2
     bt-ok AT ROW 2.42 COL 2.14
     bt-cancela AT ROW 2.42 COL 13
     bt-ajuda AT ROW 2.42 COL 58
     rt-button AT ROW 2.17 COL 1
...
ON GO OF FRAME d-vapara /* V  Para CFOP */
DO:
FIND cfop-natur NO-LOCK
    WHERE cfop-natur.cod-cfop EQ INPUT FRAME {&FRAME-NAME} c_cod-cfop NO-ERROR.

IF NOT AVAILABLE cfop-natur THEN DO:
    {utp/ut-table.i mgdis cfop-natur 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(cfop-natur).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para CFOP */
DO:  
...
------------------------------------------------------------------------------*/
  DISPLAY c_cod-cfop 
      WITH FRAME d-vapara.
  ENABLE rt-button c_cod-cfop bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "esnf016-g01" "2.06.00.001"}
