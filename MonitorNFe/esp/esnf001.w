...
*******************************************************************************/
{include/i-prgvrs.i ESNF001 2.06.00.001}
...
&Scoped-Define ENABLED-OBJECTS rt-button bt-exporta 
...
DEFINE VARIABLE h_esnf001-q01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_esnf001-v01 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
     bt-exporta AT ROW 1.38 COL 56 WIDGET-ID 2
...
ON CHOOSE OF bt-exporta IN FRAME f-cad
DO:
   ...
   RUN dispatch IN h_esnf001-q01 ('open-query':U).
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/esnf001-v01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_esnf001-v01 ).
       RUN set-position IN h_esnf001-v01 ( 2.75 , 1.43 ) NO-ERROR.
       /* Size in UIB:  ( 5.50 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/esnf001-q01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom/esnf001-z01.w,
                     ProgVaPara = esgo/esnf001-g01.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_esnf001-q01 ).
       RUN set-position IN h_esnf001-q01 ( 1.25 , 65.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_esnf001-v01. */
       RUN add-link IN adm-broker-hdl ( h_esnf001-q01 , 'Record':U , h_esnf001-v01 ).
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_esnf001-v01 ).

       /* Links to SmartQuery h_esnf001-q01. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_esnf001-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_esnf001-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_esnf001-q01 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navega ,
             bt-exporta:HANDLE IN FRAME f-cad , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-cadsim ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_esnf001-v01 ,
             bt-exporta:HANDLE IN FRAME f-cad , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESNF001" "2.06.00.001"}
