...
*******************************************************************************/
{include/i-prgvrs.i ESNF006 2.00.00.001}
...
DEFINE VARIABLE v-row-1 AS ROWID NO-UNDO.
DEFINE VARIABLE v-row-2 AS ROWID NO-UNDO.
DEFINE NEW GLOBAL SHARED VARIABLE gr-usuario-rec AS ROWID NO-UNDO.
DEFINE NEW GLOBAL SHARED VARIABLE rw-usuario-rec AS ROWID NO-UNDO.
...
DEFINE VARIABLE h_esnf006-b01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_esnf006-b02 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_esnf006-q01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_esnf006-v01 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
     bt-add AT ROW 8.25 COL 42.57
     bt-del AT ROW 9.88 COL 42.57
...
DO:
    ...
    RUN pi-posicao-query  IN h_esnf006-q01(OUTPUT v-row-1).
    RUN pi-posicao-browse IN h_esnf006-b01(OUTPUT v-row-2).
    RUN pi-add-browse     IN h_esnf006-b02(INPUT v-row-1, 
                                           INPUT v-row-2).
    RUN dispatch IN h_esnf006-b02 ('open-query':U).
...
DO:
  ...
  RUN pi-posicao-browse IN h_esnf006-b02(OUTPUT v-row-1).
  FIND nfe018 
      WHERE ROWID(nfe018) = v-row-1 EXCLUSIVE-LOCK NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/esnf006-v01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_esnf006-v01 ).
       RUN set-position IN h_esnf006-v01 ( 2.83 , 1.57 ) NO-ERROR.
       /* Size in UIB:  ( 1.50 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/esnf006-b01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_esnf006-b01 ).
       RUN set-position IN h_esnf006-b01 ( 4.58 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.00 , 40.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/esnf006-b02.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_esnf006-b02 ).
       RUN set-position IN h_esnf006-b02 ( 4.75 , 50.00 ) NO-ERROR.
       /* Size in UIB:  ( 11.75 , 40.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/esnf006-q01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = inzoom/z01in292.w,
                     ProgVaPara = ingo/g01in292.w,
                     ProgIncMod = ':U ,
             OUTPUT h_esnf006-q01 ).
       RUN set-position IN h_esnf006-q01 ( 1.00 , 53.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_esnf006-v01. */
       RUN add-link IN adm-broker-hdl ( h_esnf006-q01 , 'Record':U , h_esnf006-v01 ).

       /* Links to SmartBrowser h_esnf006-b01. */
       RUN add-link IN adm-broker-hdl ( h_esnf006-b01 , 'State':U , THIS-PROCEDURE ).

       /* Links to BrowserCadastro2 h_esnf006-b02. */
       RUN add-link IN adm-broker-hdl ( h_esnf006-q01 , 'Record':U , h_esnf006-b02 ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'State':U , h_esnf006-b02 ).

       /* Links to SmartQuery h_esnf006-q01. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_esnf006-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_esnf006-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_esnf006-q01 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_esnf006-v01 ,
             h_p-exihel , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_esnf006-b01 ,
             h_esnf006-v01 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_esnf006-b02 ,
             h_esnf006-b01 , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN rw-usuario-rec = gr-usuario-rec.

  {utp/ut9000.i "ESNF006" "2.00.00.001"} 

  /* Dispatch standard ADM method.                             */
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .

  IF rw-usuario-rec <> ? THEN 
     RUN pi-reposiciona-query IN h_esnf006-q01 (INPUT rw-usuario-rec).
  ASSIGN gr-usuario-rec = rw-usuario-rec. 
