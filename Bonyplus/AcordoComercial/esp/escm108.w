...
*******************************************************************************/
{include/i-prgvrs.i ESCM108 2.12.00.001}
...
DEFINE BUTTON btFile 
     ...
     IMAGE-INSENSITIVE FILE "image\ii-sea":U
     LABEL "" 
     SIZE 4 BY 1 TOOLTIP "Busca diret¢rio de arquivos".
...
DEFINE VARIABLE c-arquivo-entrada-1 AS CHARACTER 
     VIEW-AS EDITOR MAX-CHARS 256
     SIZE 40 BY .88
     BGCOLOR 15  NO-UNDO.
...
DEFINE VARIABLE text-entrada-2 AS CHARACTER FORMAT "X(256)":U INITIAL "Arquivo de Entrada" 
      VIEW-AS TEXT 
...
DEFINE VARIABLE rs-tipo AS INTEGER 
     ...
     RADIO-BUTTONS 
          "Exportaá∆o Dados Clientes", 1,
"Importaá∆o Dados Clientes", 2
     SIZE 27.86 BY 1.88 NO-UNDO.
...
DEFINE FRAME f-pg-sel
     i-cod-emitente-ini AT ROW 1.67 COL 19.29 COLON-ALIGNED
     i-cod-emitente-fim AT ROW 1.67 COL 42.43 COLON-ALIGNED NO-LABEL WIDGET-ID 2
     IMAGE-1 AT ROW 1.67 COL 31.72
     IMAGE-2 AT ROW 1.67 COL 41.14
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 2.83
         SIZE 75.72 BY 10 WIDGET-ID 100.
...
DEFINE FRAME f-pg-imp
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 3
         SIZE 73.72 BY 10.25 WIDGET-ID 100.

DEFINE FRAME f-pg-par
     rs-tipo AT ROW 2.63 COL 8.57 NO-LABEL WIDGET-ID 72
     btFile AT ROW 5.67 COL 45 HELP
          "Select the file name" WIDGET-ID 58
     c-arquivo-entrada-1 AT ROW 5.71 COL 4.72 HELP
          "Nome do arquivo de destino do relat¢rio" NO-LABEL WIDGET-ID 100
     c-arquivo-entrada AT ROW 8.54 COL 5 HELP
          "Nome do arquivo de destino do relat¢rio" NO-LABEL WIDGET-ID 112
     bt-arquivo-entrada AT ROW 8.54 COL 45.29 HELP
          "Escolha do nome do arquivo" WIDGET-ID 110
     text-entrada AT ROW 4.96 COL 5.86 NO-LABEL WIDGET-ID 104
     text-entrada-2 AT ROW 7.5 COL 7.29 NO-LABEL WIDGET-ID 86
     "Tipo Transaá∆o" VIEW-AS TEXT
          SIZE 14 BY .67 AT ROW 2.04 COL 6.43 WIDGET-ID 78
     RECT-8 AT ROW 5.29 COL 3.86 WIDGET-ID 102
     RECT-17 AT ROW 2.33 COL 3.86 WIDGET-ID 76
     RECT-10 AT ROW 7.83 COL 4 WIDGET-ID 114
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 3
         SIZE 75.86 BY 10.42 WIDGET-ID 100.
...
DO:
    {include/i-imarq.i c-arquivo-entrada f-pg-par "'*.csv' '*.CSV' , '*.XLS' '*.xls' , 'todos' '*.*'"}
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btFile w-relat
ON CHOOSE OF btFile IN FRAME f-pg-par
DO:
    /*******************************************************/
    /* Abertura da caixa de pesquisa para busca de arquivo */
    /* em diret¢rio                                        */
    /*******************************************************/
    DEFINE VARIABLE c-diretorio-new  AS CHAR NO-UNDO.

    INPUT FROM OS-DIR("C:\")CONVERT SOURCE "iso8859-1".

    SYSTEM-DIALOG GET-DIR c-diretorio-new
    INITIAL-DIR "C:\" 
    RETURN-TO-START-DIR 
...
/*                                                                 */
    ASSIGN c-arquivo-entrada-1 :SCREEN-VALUE IN FRAME f-pg-par = c-diretorio-new.
...
ON ENTRY OF rs-tipo IN FRAME f-pg-par
DO:
    IF rs-tipo:SCREEN-VALUE IN FRAME f-pg-par = "1" THEN 
        ASSIGN c-arquivo-entrada-1:SCREEN-VALUE = "".
    
    ELSE 
        ASSIGN c-arquivo-entrada:SCREEN-VALUE = "".
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL rs-tipo w-relat
ON VALUE-CHANGED OF rs-tipo IN FRAME f-pg-par
DO:
    
    IF rs-tipo:SCREEN-VALUE IN FRAME f-pg-par = "2" THEN DO:
        DISABLE c-arquivo-entrada-1
                btFile 
                text-entrada WITH FRAME {&FRAME-NAME}. 

        DISABLE i-cod-emitente-ini
                i-cod-emitente-fim WITH FRAME f-pg-sel.

        ASSIGN c-arquivo-entrada-1:SCREEN-VALUE  = ""
               i-cod-emitente-ini:SCREEN-VALUE = "0"
               i-cod-emitente-fim:SCREEN-VALUE = "999999999" .

        ENABLE c-arquivo-entrada 
               bt-arquivo-entrada 
               text-entrada-2 WITH FRAME {&FRAME-NAME}.
    END.
        ELSE DO:
            ENABLE c-arquivo-entrada-1 
                   btFile 
                   text-entrada WITH FRAME {&FRAME-NAME}.
...
{utp/ut9000.i "ESCM108" "2.12.00.001"}
...
DO  ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    APPLY 'value-changed' TO rs-tipo IN FRAME f-pg-par.
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY i-cod-emitente-ini i-cod-emitente-fim 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
  ENABLE IMAGE-1 IMAGE-2 i-cod-emitente-ini i-cod-emitente-fim 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
  ...
  DISPLAY rs-tipo c-arquivo-entrada-1 c-arquivo-entrada 
      WITH FRAME f-pg-par IN WINDOW w-relat.
  ENABLE RECT-8 RECT-17 RECT-10 rs-tipo btFile c-arquivo-entrada-1 
...
do on error undo, return error on stop  undo, return error:
    ...
    IF INPUT FRAME f-pg-par rs-tipo = 1 THEN DO:

        IF INPUT FRAME f-pg-par  c-arquivo-entrada-1  = "" THEN DO:
        
           RUN utp/ut-msgs.p (INPUT "show",
                              INPUT 15825,
                              INPUT "Diret¢rio de Sa°da para Exportaá∆o Inv†lido !!!"
                              + "~~" + "Favor Informar o Diret¢rio de Sa°da para Exportaá∆o").
           APPLY "entry" TO INPUT c-arquivo-entrada-1  IN FRAME f-pg-par.
           RETURN 'adm-error':u.
        END.

        FILE-INFO:FILE-NAME = c-arquivo-entrada-1:SCREEN-VALUE IN FRAME f-pg-par.
        
        IF FILE-INFO:FULL-PATHNAME = ? THEN DO:
               RUN utp/ut-msgs.p (INPUT "show",
                                  INPUT 15825,
                                  INPUT "Diret¢rio de Sa°da Inv†lido !!!"
                                  + "~~" + "Favor Informar um Diret¢rio Existente para Exportaá∆o").
               APPLY "entry" TO INPUT c-arquivo-entrada-1  IN FRAME f-pg-par.
               RETURN 'adm-error':u.
        END.
    END.
    ELSE DO:
        IF INPUT FRAME f-pg-par  c-arquivo-entrada  = "" THEN DO:
           ...
           RUN utp/ut-msgs.p (INPUT "show",
                              INPUT 15825,
                              INPUT "Diret¢rio de Entrada para Importaá∆o Inv†lido !!!"
                              + "~~" + "Favor Informar o Diret¢rio de Entrada para Importaá∆o!").
           APPLY "entry" TO INPUT c-arquivo-entrada  IN FRAME f-pg-par.
           RETURN 'adm-error':u.
        END.
        ...
        IF FILE-INFO:FULL-PATHNAME = ? THEN DO:
               RUN utp/ut-msgs.p (INPUT "show",
                                  INPUT 15825,
                                  INPUT "Diret¢rio de Entrada Inv†lido !!!"
                                  + "~~" + "Favor Informar um Diret¢rio Existente para Importaá∆o!").
               APPLY "entry" TO INPUT c-arquivo-entrada  IN FRAME f-pg-par.
               RETURN 'adm-error':u.
        END.
    END.
    ...
    assign tt-param.usuario         = c-seg-usuario
           ...
           &endif
           tt-param.rs-tipo                    = INPUT FRAME f-pg-par rs-tipo 
           tt-param.arq-entrada                = INPUT FRAME f-pg-par c-arquivo-entrada-1 
           tt-param.arq-entrada-2              = INPUT FRAME f-pg-par c-arquivo-entrada
    ...
    {include/i-rprun.i esp/escm108rp.p}
