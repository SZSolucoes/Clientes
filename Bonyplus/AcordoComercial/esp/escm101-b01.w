...
*******************************************************************************/
{include/i-prgvrs.i ESCM101-B01 2.12.00.001}
...
&Scoped-define INTERNAL-TABLES es-acordo-area

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br-table                                      */
&Scoped-define FIELDS-IN-QUERY-br-table es-acordo-area.cod-area ~
es-acordo-area.descricao 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br-table 
&Scoped-define QUERY-STRING-br-table FOR EACH es-acordo-area WHERE ~{&KEY-PHRASE} ~
      AND es-acordo-area.cod-area >= i-inicial ~
 AND es-acordo-area.cod-area <= i-final NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-br-table OPEN QUERY br-table FOR EACH es-acordo-area WHERE ~{&KEY-PHRASE} ~
      AND es-acordo-area.cod-area >= i-inicial ~
 AND es-acordo-area.cod-area <= i-final NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-br-table es-acordo-area
&Scoped-define FIRST-TABLE-IN-QUERY-br-table es-acordo-area
...
DEFINE VARIABLE i-final AS INTEGER FORMAT ">>>>>9" INITIAL 999999 
     VIEW-AS FILL-IN 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-table B-table-Win _STRUCTURED
  QUERY br-table NO-LOCK DISPLAY
      es-acordo-area.cod-area FORMAT ">>>>>9":U
...
DEFINE FRAME F-Main
     ...
     i-inicial AT ROW 1.08 COL 20.86 COLON-ALIGNED
     i-final AT ROW 1.08 COL 40.29 COLON-ALIGNED NO-LABEL
     br-table AT ROW 2.17 COL 1
     IMAGE-1 AT ROW 1.08 COL 33
     IMAGE-2 AT ROW 1.08 COL 38.86
...
/* Query rebuild information for BROWSE br-table
     _TblList          = "ems2custom.es-acordo-area"
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE SORTBY-PHRASE"
     _Where[1]         = "ems2custom.es-acordo-area.cod-area >= i-inicial
 AND ems2custom.es-acordo-area.cod-area <= i-final"
     _FldNameList[1]   = ems2custom.es-acordo-area.cod-area
     _FldNameList[2]   = ems2custom.es-acordo-area.descricao
...
DEFINE INPUT PARAMETER P-CAMPO AS CHARACTER NO-UNDO.
    ...
    if  avail ems2custom.es-acordo-area then do:
        case p-campo:
            when "cod-area" then
                assign p-valor = string(es-acordo-area.cod-area).
            when "descricao" then
