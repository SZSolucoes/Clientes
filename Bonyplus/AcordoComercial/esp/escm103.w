...
*******************************************************************************/
{include/i-prgvrs.i ESCM103 2.12.00.001}
...
DEFINE VARIABLE h_escm103-q01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_escm103-v01 AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/escm103-v01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_escm103-v01 ).
       RUN set-position IN h_escm103-v01 ( 3.00 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.25 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/escm103-q01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = esp/escm103-z01.w,
                     ProgVaPara = esp/escm103-g01.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_escm103-q01 ).
       RUN set-position IN h_escm103-q01 ( 1.38 , 65.86 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_escm103-v01. */
       RUN add-link IN adm-broker-hdl ( h_escm103-q01 , 'Record':U , h_escm103-v01 ).
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_escm103-v01 ).

       /* Links to SmartQuery h_escm103-q01. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_escm103-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_escm103-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_escm103-q01 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_escm103-v01 ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESCM103" "2.12.00.001"}
